...
gulp.task('watch', function(){
  ...
  // JS
  $.watch(config.appjs, {events: ['change']}, $.batch(function(events){
    events.on(['change'], runSequence('scripts', reload));
  }));
  $.watch(config.appjs, {events: ['unlink', 'add']}, $.batch(function(events){
    events.on(['unlink','add'], runSequence( 'build'));
  }))
  .on(['add', 'change', 'unlink'],  $.memoryCache.update('appjs'));
  // ASSETS
